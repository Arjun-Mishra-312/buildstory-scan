...
| AI session prompts, responses, reasoning (Codex, Claude Code, Cursor, Google Antigravity) | JSON/SQLite record materialized locally; content ignored | Never; discard count only | Never |
...
| BYOK API key (`BUILDSTORY_BYOK_API_KEY`) | Read from the environment only, never a CLI flag | Never | Never - sent only as the Authorization header on the creator's own request to their configured provider, which is not a Buildstory endpoint |
| BYOK narrative excerpts | Selected and redacted the same as local mode | Never | Sent only to the creator's own configured provider, never to Buildstory |

`inspect` never calls the network. `scan` will not read session roots until the exact `--consent local-scan` is present. In local or bring-your-own-key (BYOK) narrative mode, the scanner may call Ollama or the creator's configured provider over the network for narrative generation; in both cases this is not a Buildstory transmission, and conversation excerpts (redacted the same way in both modes) are never uploaded - only the resulting narrative prose is.
...
`scan-upload` is the only non-loopback data plane. It requires both local-scan and local-dashboard upload consent, then sends only the already validated canonical `ProjectSnapshot`. In local and BYOK mode the upload can contain `generatedNarrative`, but never `narrativeEvidence`; in cloud mode the reviewed, redacted `narrativeEvidence` excerpts are the explicit opt-in exception. A dashboard-selected mode is authoritative for the whole connection: `--with-evidence` can never override a local, BYOK, or off connection into uploading excerpts - a mismatch is refused (`NARRATIVE_MODE_CONFLICT`) rather than silently resolved. No raw input, adapter object, repository path, environment dump, error detail, cookie, multipart attachment, wrapper object, or BYOK API key can reach the transport API.
