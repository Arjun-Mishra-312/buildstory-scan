...
- `UPLOAD_UNAVAILABLE`: no automatic retry occurs. The local grant remains available for one manual retry, although the server will refuse it if the ambiguous first request was already accepted.
...
- **Local** (the default): calls Ollama on loopback. Its standard report is not plan-gated and automatically selects a safe, balanced, or enhanced evidence profile from available RAM and logical CPUs: up to 40, 64, or 80 excerpts respectively. Set `BUILDSTORY_OLLAMA_BASE_URL` to override the default `http://127.0.0.1:11434`.
- **Bring your own key (BYOK)**: choose OpenRouter/DeepSeek with `BUILDSTORY_OPENROUTER_API_KEY` or OpenAI/Luna with `BUILDSTORY_OPENAI_API_KEY`. Optional provider-specific overrides are `BUILDSTORY_OPENROUTER_BASE_URL`, `BUILDSTORY_OPENROUTER_MODEL`, `BUILDSTORY_OPENAI_BASE_URL`, and `BUILDSTORY_OPENAI_MODEL`. Legacy `BUILDSTORY_BYOK_API_KEY`, `BUILDSTORY_BYOK_BASE_URL`, and `BUILDSTORY_BYOK_MODEL` remain supported. Keys are read only from the scanner environment, never a CLI flag or browser request. `--review` is mandatory and occurs before any provider request. Standard uses the compact pipeline and is capped at 80 excerpts/800 characters each/60,000 characters total; Deep uses a private analysis pass followed by V3 synthesis, with at most one bounded JSON-repair request per component, and is capped at 240 excerpts/1,500 characters each/700 KiB total, dynamically reduced to remain below the upload grant. OpenRouter calls require ZDR routing and deny data collection; OpenAI calls send `store: false`, while retention follows the user's OpenAI organization policy.
- **Buildstory Cloud**: requires `--with-evidence --review`. Standard is capped at 80 excerpts, 800 characters each and 60,000 characters total. An eligible deep session is capped at 240 excerpts, 1,500 characters each and 700 KiB total, dynamically reduced within the 1 MiB upload grant. The reviewed excerpts and disclosed deterministic facts go to DeepSeek V4 Flash through OpenRouter with a ZDR-eligible endpoint required and data collection denied.
